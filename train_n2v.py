...
class Train_n2v:
    ...
    def train_n2v(self):
        ...
        transform_train = transforms.Compose([Normalize(mean=0.5, std=0.5), RandomFlip(), ToTensor()])
        transform_val = transforms.Compose([Normalize(mean=0.5, std=0.5), RandomFlip(), ToTensor()])
        transform_inv = transforms.Compose([ToNumpy(), Denormalize(mean=0.5, std=0.5)])
        dataset_train = N2VDataset(dir_data_train, data_type=self.data_type, transform=transform_train, sgm=20,
                                ratio=self.ratio, size_data=size_data, size_window=size_window)
        dataset_val = N2VDataset(dir_data_val, data_type=self.data_type, transform=transform_val, sgm=20,
                              ratio=self.ratio, size_data=size_data, size_window=size_window)
        ...
        netG2 = Net1(1, 1)
        netG3 = Net2(2, 1)
        inshape = (self.nch_in,self.nx_in,self.ny_in)
        ...
        optimG1 = torch.optim.Adam(paramsG1, lr=2e-4)
        ...
        if train_continue == 'on':
            netG1, optimG1, st_epoch = self.load(dir_chck, netG1, optimG1,'G1', mode=mode)
            netD, optimDis, st_epoch = self.load(dir_chck, Dis, optimDis, 'Dis', mode=mode)
            netG2, optimG2, st_epoch = self.load(dir_chck, netG2, optimG2, 'G2', mode=mode)
            netG3, optimG3, st_epoch = self.load(dir_chck, netG3, optimG3, 'G3', mode=mode)
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for batch, data in enumerate(loader_train, 1):
                ...
                valid = Variable(torch.ones((label.size(0),1)), requires_grad=False).to(device)  ## 定义真实的图片label为1 ones((1, 1, 16, 16))
                fake = Variable(torch.zeros((label.size(0),1)), requires_grad=False).to(device)
                # backward
                ...
                loss_Dis = -torch.mean(Dis(label * (1 - mask)))+torch.mean(Dis(out1d * (1 - mask)))+ 0.5*gp
                ...
                optimG2.zero_grad()
                out2 = netG2(input)
                # loss_G2 = criterion(out2 * (1 - mask), label * (1 - mask))
                loss_G2 = criterion(out2 * (1 - mask), label * (1 - mask))+vgg_loss(out2 * (1 - mask), label * (1 - mask))
                loss_G2.backward()
                optimG2.step()
                ...
                G3loss = criterion(output * (1 - mask), label * (1 - mask))+vgg_loss(output * (1 - mask), label * (1 - mask))
                loss_G3 = G3loss
                loss_G3.backward()
                optimG3.step()
                ...
                if should(num_freq_disp):
                    ...
                    writer_train.add_images('origin', origin, num_batch_train * (epoch - 1) + batch, dataformats='NCHW')
            ...
            with torch.no_grad():
                ...
                for batch, data in enumerate(loader_val, 1):
                    ...
                    valid = Variable(torch.ones((label.size(0),1)), requires_grad=False).to(
                        device)  ## 定义真实的图片label为1 ones((1, 1, 16, 16))
                    fake = Variable(torch.zeros((label.size(0),1)), requires_grad=False).to(device)
                    # get losses
                    out1d = netG1(input).detach()
                    loss_Dis = -torch.mean(Dis(label * (1 - mask))) + torch.mean(Dis(out1d * (1 - mask))) + 0 * gp
                    ...
                    if should(num_freq_disp):
                        ...
                        writer_val.add_images('origin', origin, num_batch_val * (epoch - 1) + batch, dataformats='NCHW')
    ...
    def test_n2v(self):
        ...
        dataset_test = N2VDataset(dir_data_test, data_type=self.data_type, transform=transform_test, sgm=25, ratio=1, size_data=size_data, size_window=size_window)
        ...
        with torch.no_grad():
            ...
            result = np.zeros((self.nx_in, self.ny_in, len(loader_test), 2))
            for i, data in enumerate(loader_test, 1):
                ...
                out2 = netG2(input)
                output = netG3(out1.detach(), out2.detach())
                input = transform_inv(origin)
                output = transform_inv(output)
                out1 = transform_inv(out1)

                # input = np.clip(input, 0, 1)
                # output = np.clip(output, 0, 1)
                result[:, :, i - 1, 0] = input[0, :, :, :].squeeze()
                result[:, :, i - 1, 1] = output[0, :, :, :].squeeze()
            result=norm(result)
            for j in range(result.shape[-2]):
                cv2.imwrite(os.path.join(dir_result_test, 'images', f'image_{j}.png'), result[:, :, j,1])
            fig = figshow2(result)
